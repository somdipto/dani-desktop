...
# Dani Bot 0.1.7 installed the application ancestors as 0775, which makes
...
if [ -n "${DANIBOT_POSTINSTALL_TEST_ROOT:-}" ]; then
  ...
  if [ -L /opt ] || [ "$(stat -c '%U:%G:%a' -- /opt)" != root:root:755 ]; then
    echo "Dani Bot package ancestor is unsafe: /opt must be root:root 0755" >&2
    exit 1
  fi
fi
...
secure_browser_tree() {
  ...
  if [ -n "$unsafe" ]; then
    echo "Dani Bot browser resource is missing or unsafe: $unsafe" >&2
    exit 1
  fi
  unsafe=$(find "$BROWSER_ROOT" -type f -links +1 -print -quit)
  if [ -n "$unsafe" ]; then
    echo "Dani Bot browser resource has an unsafe hard link: $unsafe" >&2
    exit 1
  fi
...
install_browser_apparmor_profile() {
  ...
  if [ -f "$USERNS_RESTRICTION" ]; then
    ...
    if ! restricted=$(cat -- "$USERNS_RESTRICTION"); then
      echo "Dani Bot could not read the browser user-namespace restriction." >&2
      exit 1
    fi
    case "$restricted" in
      0|1) ;;
      *)
        echo "Dani Bot browser user-namespace restriction is invalid; refusing unsafe sandbox setup." >&2
        exit 1
        ;;
    esac
  fi
  ...
  if [ ! -d "$APPARMOR_DIR" ] || [ ! -x "$APPARMOR_PARSER" ]; then
    if [ "$restricted" = 1 ]; then
      echo "Dani Bot needs apparmor and apparmor_parser to configure the browser sandbox on this host." >&2
      exit 1
    fi
    return
  fi
  ...
  if [ ! -x "$APPARMOR_STATUS" ] || ! "$APPARMOR_STATUS" --enabled >/dev/null 2>&1; then
    if [ "$restricted" = 1 ]; then
      echo "Dani Bot cannot load its browser sandbox policy while AppArmor restrictions are active but AppArmor is unavailable." >&2
      exit 1
    fi
    return
  fi
