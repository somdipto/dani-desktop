...
```bash
sudo apt-get install -y '/home/you/.cache/openmausbot-updater/pending/Dani Bot-<version>-amd64.deb'
```
...
The installed version runs the updater, not the version being downloaded. If you are upgrading from an older build with a stuck updater, you may need to install the new version manually once to receive the fix. Do not delete your Dani Bot data directory.
...
```bash
sudo apt-get install -y ~/.cache/openmausbot-updater/pending/Dani Bot-*-amd64.deb
```
