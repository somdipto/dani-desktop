...
3. Restart Dani Bot after installation.
...
Run the provider's own status or login command in a terminal. Dani Bot uses the CLI's authenticated session; installing the binary alone is not enough.

A `401 Unauthorized` response from `api.openai.com` means the launched Codex process did not receive valid authentication. Confirm `codex login status`, sign in again if needed, then restart Dani Bot. Do not paste a ChatGPT password into Dani Bot.
...
- Try the provider CLI directly with the same model to separate an account problem from an Dani Bot integration problem.
