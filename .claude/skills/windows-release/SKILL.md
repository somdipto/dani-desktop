...
Ships `Dani Bot-<version>-setup.exe` and its update feed to
...
| File | Purpose |
|---|---|
| `Dani Bot-<version>-setup.exe` | the installer |
| `latest.yml` | **the update feed** — see step 4 |
| `Dani Bot-<version>-setup.exe.blockmap` | differential updates |
| `Dani Bot-<version>-x64.zip` | portable, not used by the updater |
...
```powershell
Copy-Item release/Dani Bot-<version>-setup.exe release/Dani Bot-setup.exe
gh release upload v<version> --repo somdipto/dani-desktop `
  release/Dani Bot-<version>-setup.exe `
  release/Dani Bot-setup.exe `
  release/Dani Bot-<version>-setup.exe.blockmap `
  release/latest.yml
```
...
to the matching draft in `somdipto/dani-desktop`; never replace the
bytes of an already-published asset.

Both names are required, for different consumers:

- **`Dani Bot-<version>-setup.exe`** is what `latest.yml` references by name and
  sha512. The auto-updater downloads exactly this.
- **`Dani Bot-setup.exe`** is a byte-identical copy that gives the README's
  `/releases/latest/download/Dani Bot-setup.exe` button a stable URL. This
  mirrors `Dani Bot.dmg` sitting beside `Dani Bot-<version>.dmg`.
