...
/// Floating overlay: tiny capsule at rest, waveform while listening, spinner
/// while transcribing/thinking/working, status text for DANI states.
///
/// Renamed from Scribe's `OverlayPanel` — same native NSPanel behavior
/// (borderless, non-activating, floats on all spaces). The DANI execution
/// states (Understanding… / Working… / Done ✓ / etc.) are layered on top in
/// a later commit; the waveform + transcript pill + spinner primitives here
/// are Scribe's and stay unchanged.
final class DaniOverlay: NSPanel {
