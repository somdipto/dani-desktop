...
///
/// Renamed from Scribe's `MicrophoneRouter` — same behavior, DANI name.
final class VoiceCapture {
    static let shared = VoiceCapture()

    /// Posted on the main queue when the input device list changes.
    static let devicesChanged = Notification.Name("dani.mic.devicesChanged")
    ...
    func startObservingDeviceChanges() {
        ...
        let block: AudioObjectPropertyListenerBlock = { _, _ in
            ...
            NotificationCenter.default.post(name: VoiceCapture.devicesChanged, object: nil)
        }
