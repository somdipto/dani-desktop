...
///
/// Renamed from Scribe's `AppleSpeechSession` — same behavior, DANI name.
/// This is one of Scribe's native primitives DANI keeps unchanged.
final class SpeechTranscriber {
    ...
    func start() {
        ...
        if let deviceID = VoiceCapture.shared.resolvedDeviceID() {
            do {
                try audioEngine.inputNode.auAudioUnit.setDeviceID(deviceID)
            } catch {
                NSLog("Dani mic: failed to set input device %u: %@", deviceID, error.localizedDescription)
            }
        }
