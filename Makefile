...
#                ENTITLEMENTS=Dani.entitlements
CODESIGN_IDENTITY ?= -
ENTITLEMENTS ?=
...
.PHONY: build clean install run release dmg install-hooks
...
build:
	...
	cp $(BUILD_DIR)/$(APP_NAME)App $(APP_BUNDLE)/Contents/MacOS/
	...
	@# Sign any embedded frameworks a future dependency might bring. No-op
	@# today (Dani ships no embedded frameworks), kept so adding one later
	@# doesn't require touching the Makefile.
