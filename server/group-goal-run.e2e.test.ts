...
const completeReplies = [
  [
    "Scout should verify the draft.\n<danibot-goal>{\"status\":\"continue\",",
    "\"next\":\"Scout\",\"instruction\":\"Verify the draft and report evidence\",\"detail\":\"Draft prepared\"}</danibot-goal>",
  ],
  "The draft is accurate and the cited evidence checks out.",
  "The verified draft is ready to ship.\n<danibot-goal>{\"status\":\"completed\",\"detail\":\"Draft produced and independently verified.\"}</danibot-goal>",
];

const loopReplies = Array.from({ length: 13 }, (_, index) =>
  index % 2 === 0
    ? `More work is needed.\n<danibot-goal>{"status":"continue","next":"Looper","instruction":"Try approach ${index / 2 + 1}","detail":"Still working"}</danibot-goal>`
    ...
    instances: {
      ...
      busyGoal: {
        ...
        environment: {
          ...
          FAKE_CLAUDE_REPLIES: JSON.stringify([
            ...
            "The queued team goal is complete.\n<danibot-goal>{\"status\":\"completed\",\"detail\":\"Waited for the lead, then completed normally.\"}</danibot-goal>",
          ]),
      ...
      busyWorkerLead: {
        ...
        environment: {
          FAKE_CLAUDE_MODE: "happy",
          FAKE_CLAUDE_REPLIES: JSON.stringify([
            "I am delegating the research.\n<danibot-goal>{\"status\":\"continue\",\"next\":\"Busy specialist\",\"instruction\":\"Research the answer and report evidence\",\"detail\":\"Waiting for specialist research.\"}</danibot-goal>",
            "The specialist's evidence resolves the goal.\n<danibot-goal>{\"status\":\"completed\",\"detail\":\"Specialist research incorporated after their direct task finished.\"}</danibot-goal>",
          ]),
      ...
      stopScopedLead: {
        ...
        environment: {
          ...
          FAKE_CLAUDE_REPLIES: JSON.stringify([
            "I am delegating this scheduled goal.\n<danibot-goal>{\"status\":\"continue\",\"next\":\"Delayed worker\",\"instruction\":\"Finish the scheduled analysis\",\"detail\":\"Waiting for the delayed worker.\"}</danibot-goal>",
            "This is unrelated direct work and should be stopped.",
            "The scheduled analysis is now complete.\n<danibot-goal>{\"status\":\"completed\",\"detail\":\"Scheduled goal survived the coordinator's direct Stop.\"}</danibot-goal>",
          ]),
      ...
      routineGoal: {
        ...
        environment: {
          ...
          FAKE_CLAUDE_REPLIES: JSON.stringify([
            "The scheduled review is complete.\n<danibot-goal>{\"status\":\"completed\",\"detail\":\"Scheduled team review completed.\"}</danibot-goal>",
          ]),
...
describe("goal-driven channel runs", () => {
  it("returns to the lead until the goal is completed and appends a clean receipt", async () => {
    ...
    expect(JSON.stringify(current.messages)).not.toContain("<danibot-goal>");
  });
