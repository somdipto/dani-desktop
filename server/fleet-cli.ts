// `danibot fleet`: many client workspaces on one Linux server. The plans
...
export async function executePlan(steps: FleetStep[], deps: FleetDeps, io: FleetIo): Promise<number> {
  for (const step of steps) {
    switch (step.kind) {
      ...
      case "health":
        if (!(await deps.health(step.url, HEALTH_TIMEOUT_MS))) return failed(io, step.why, `${step.url} did not answer within ${HEALTH_TIMEOUT_MS / 1000}s; check journalctl -u danibot@<slug>`);
        break;
      case "note":
        io.log(step.text);
        break;
...
export function loadRegistry(layout: FleetLayout, deps: FleetDeps): FleetRegistry {
  ...
  if (text === null) throw new Error(`no fleet on this server yet: run \`danibot fleet init --domain your.domain\` first`);
...
export async function runFleetCommand(input: FleetInput, io: FleetIo, deps: FleetDeps = defaultFleetDeps()): Promise<number> {
  ...
  try {
    if (input.action === "list") {
      ...
      if (!rows.length) {
        io.log(`no workspaces yet on ${registry.domain}; create one with: danibot fleet create NAME --admin you@example.com`);
        return 0;
      }
      for (const workspace of rows) {
        const live = asRoot ? (await deps.run(["systemctl", "is-active", `danibot@${workspace.slug}.service`])).output.trim() : workspace.status;
