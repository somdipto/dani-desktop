...
//
//   danibot setup [--data-dir ~/.danibot]
//   danibot start [serve options]
...
//   danibot status
//   danibot login [--email you@example.com]
//   danibot logout
//
...
// `--tunnel` (after `login`) serves at a public https://….example.com
...
export const USAGE = `danibot — your team of AI bots, ready in a few steps

  danibot                         set up once, then open your workspace
  danibot setup [--data-dir DIR]
  danibot start [the same options as serve]
  ...
  danibot login [--email you@example.com]
  danibot logout
  danibot access list | add EMAIL [--chat-only] | remove EMAIL
  danibot service install [--domain HOST | --tunnel | --tailscale] [--port N] [--data-dir DIR] | uninstall
  danibot browser install [--with-deps] | status
  danibot fleet init --domain HOST [--operator USER] | create NAME --admin EMAIL [--member EMAIL] [--brand FILE]
...
start   same as danibot: use your saved settings and open the workspace
...
login   signs this machine in to an Dani Bot account (an emailed code)
...
--tunnel     serve at a public https://….example.com address through a
             Cloudflare tunnel: no domain, no proxy, no open port. Run
             \`danibot login\` once on this machine first.
...
Install once with \`npm install -g danibot\`, then type \`danibot\`.
Or run without a global install: \`npx danibot\`. Node 24+ is required.
`;
...
export async function runStatus(options: CliOptions, io: CliIo = defaultIo()): Promise<number> {
  ...
  try {
    ...
    io.error(`no Dani Bot server on http://127.0.0.1:${options.port}`);
    code = 1;
  }
  if (!options.json) {
...
export async function runLogin(options: CliOptions, io: CliIo = defaultIo()): Promise<number> {
  ...
  io.log("Serve there with:  danibot serve --tunnel");
  return 0;
}
...
export async function runBrowser(options: CliOptions, io: CliIo = defaultIo()): Promise<number> {
  ...
  if (options.browserAction === "status") {
    ...
    if (status.kind !== "ready" && status.installable) io.log("install it with:  danibot browser install");
...
export async function runServe(options: CliOptions, log: (line: string) => void = console.log): Promise<number> {
  ...
  try {
    ...
    if (exited !== null) {
      if (exited !== 0) log(`Dani Bot could not start.${logPath ? ` Details: ${logPath}` : " See the output above."}`);
      return exited;
    }
    if (stopping) return await childExit;
    if (!(await serverUp(options.port, child.pid))) {
      console.error(`Dani Bot did not become ready within a minute.${logPath ? ` Details: ${logPath}` : " See its output above."}`);
      await stop();
      return 1;
    }
...
export async function runOnboardingCommand(
  ...
  if (options.command === "setup" && !interactive) {
    io.error("Setup needs an interactive terminal. Run `npx danibot setup` in a terminal, then use `npx danibot serve` for unattended starts.");
    return 1;
  }
  ...
  try {
    if (options.command === "setup" || !(await isSetupComplete(options.dataDir))) {
      if (!interactive) {
        io.error("No completed setup was found. Run `npx danibot setup` in an interactive terminal first, or use `npx danibot serve` with an existing configuration.");
        return 1;
      }
