...
export async function listManagedBoxes(
  ...
  for (const candidate of listed.boxes) {
    ...
    if (!BOX_ID.test(boxId)) {
      return invalidInventory("ascii.dev returned an invalid id for a Dani Bot-managed cloud computer — refresh or repair it in ascii.dev");
    }
  ...
  for (const candidate of listed.boxes) {
    ...
    if ((boxIdCounts.get(boxId) ?? 0) !== 1 || seenBoxIds.has(boxId)) {
      return invalidInventory("ascii.dev returned a conflicting id for a Dani Bot-managed cloud computer — refresh or repair it in ascii.dev");
    }
