...
export function createBotPackageExport(input: {
  ...
  const definition: BotPackageDefinition = {
    ...
    release: "1.0.0",
    name: input.name,
    tagline: `A portable Dani Bot setup with ${bots.length} ${bots.length === 1 ? "bot" : "bots"}.`,
    summary: "Exported from Dani Bot. Review the roles, rooms, playbooks, connector requirements, and paused routines before sharing or publishing.",
    category: "Community",
    author: { name: input.authorName?.trim() || "Dani Bot user" },
    license: "Unspecified",
