...
function cachePath(capsule: TestCapsule): string {
  const root = mkdtempSync(join(tmpdir(), "danibot-status-"));
  roots.push(root);
...
posixOnly("readDaniStatus", () => {
  it("projects only fresh normalized two-VM capability data", () => {
    ...
    // Cross-language receipt produced by scripts/aos_danibot_status.py
  ...
  it("rejects a non-0700 parent and a symlinked parent", () => {
    ...
    const targetPath = cachePath(successCapsule());
    const linkRoot = mkdtempSync(join(tmpdir(), "danibot-status-parent-link-"));
    roots.push(linkRoot);
