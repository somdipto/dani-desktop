package com.danibot.companion.core
...
class ConnectionTest {
    ...
    fun parsersNormalizeNonIpv6ZoneSuffixes() {
        ...
        val invite = PairingInvite.parse(
            URI("danibot://pair?address=192.168.1.3%25en0%3A9910&code=004209"),
        )
    ...
    fun parsesADesktopPairingInvite() {
        ...
        val invite = PairingInvite.parse(
            URI("danibot://pair?address=macbook.tail1234.ts.net%3A8810&token=$token&code=004209&name=Milind%27s%20Mac"),
        )!!
    ...
    fun literalPlusInPairingInviteNameIsPreserved() {
        val invite = PairingInvite.parse(
            URI("danibot://pair?address=mac.local&code=004209&name=Ada%27s+Mac"),
        )
    ...
    fun parsesAnOlderCodeOnlyPairingInvite() {
        val invite = PairingInvite.parse(URI("danibot://pair?address=mac.local&code=004209"))
    ...
    fun typedInviteEstablishesPolicyFromItsSelectedEndpoint() {
        ...
        val invite = assertNotNull(PairingInvite.parse(URI(
            "danibot://pair?address=192.168.1.42%3A8810&token=$token&endpoints=${base64Url(routes)}",
        )))
    ...
    fun presentButInvalidTypedEndpointsRejectTheWholeInvite() {
        ...
        assertNull(PairingInvite.parse(URI(
            "danibot://pair?address=192.168.1.42%3A8810&token=$token&endpoints=",
        )))
        assertNull(PairingInvite.parse(URI(
            "danibot://pair?address=192.168.1.42%3A8810&token=$token&endpoints=${"a".repeat(8_193)}",
        )))
    }
    ...
    fun dropsUnusableFallbackHostsWithoutRefusingTheInvite() {
        val invite = PairingInvite.parse(URI(
            "danibot://pair?address=mac.local&code=004209&hosts=%20192.168.1.42%20,,bad%2Fslash,has%20space",
        ))!!
        assertEquals(emptyList(), invite.connection.hosts)
        val empty = PairingInvite.parse(
            URI("danibot://pair?address=mac.local&code=004209&hosts=bad%2Fslash"),
        )
    ...
    fun rejectsUntrustedOrMalformedPairingInvites() {
        listOf(
            ...
            "danibot://pair?address=mac.local&code=12345",
            "danibot://pair?address=mac.local&token=weak",
            "danibot://pair?address=mac.local&token=weak&code=123456",
            "danibot://pair?address=host%2Fpath&code=123456",
            "danibot://pair?address=one.local&address=two.local&code=123456",
