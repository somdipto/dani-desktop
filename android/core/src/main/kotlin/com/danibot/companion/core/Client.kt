...
class PairingRouteError(val attemptedRoutes: List<String>) : IOException(
    "Couldn't reach this computer through any available route " +
        "(${attemptedRoutes.joinToString()}). Keep Phone access turned on in Dani Bot, then try again.",
)

internal const val SCOPED_IPV6_HTTP_HOST = "scoped-ipv6.danibot.invalid"
    ...
    suspend fun cancelQueued(queueId: String, to: MessageDestination) {
        ...
        try {
            ...
            throw APIError.Status(
                404,
                "This computer is too old to take back a queued message. Update Dani Bot on it.",
            )
        }
    }
