...
Run the Dani Bot server, Caddy, and per-bot Linux desktops with one Podman
...
```text
Browser -> loopback Caddy :8080 -> Dani Bot :8799
                                    |
...
Mint a pairing code with `maus.ps1 exec omb node dist-server/danibot.js pair`.
