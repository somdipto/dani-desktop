...
AppArmor policy for the root-owned browser executable under `/opt/Dani Bot`.
...
On macOS, use `/absolute/Dani Bot.app/Contents/Resources`. On Linux, run as
an unprivileged user against the installed `.deb` at
`/opt/Dani Bot/resources`. This check creates its own empty home and local
...
Windows uses an explicitly identified Dani Bot build of agent-browser
