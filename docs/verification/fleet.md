...
```sh
danibot fleet init --domain example.test --dry-run
danibot fleet create acme --admin owner@acme.test --dry-run
```
...
1. Fresh Ubuntu 24.04 or newer, Caddy from apt, `npm install -g danibot`,
   Claude Code installed once as root, a wildcard DNS record at the server.
2. `danibot fleet init --domain <domain>`; check `systemctl status
   danibot-fence` and `nft list table inet danibot`.
3. `danibot fleet create alpha --admin you@example.test --cap 1` and the
...
4. Isolation: as `omb-alpha` (`runuser -u omb-alpha -- curl -s
   ...
   as root it must answer. `ls /var/lib/danibot/beta` as `omb-alpha` must
   be denied.
