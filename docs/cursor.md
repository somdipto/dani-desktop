...
Cursor is an optional Dani Bot engine. Dani Bot runs the official
...
3. Confirm `cursor-agent --version` works. The binary installs to `~/.local/bin` by
   default; Dani Bot already looks there when launched from a GUI.
...
CLI also implements ACP `session/set_model`, Dani Bot pins the same id over
...
does not provide a bot approval level. Dani Bot app turns always provide
one: both **Ask for approval** and **Approve for me** launch Cursor without
`--force`, then Dani Bot handles its permission requests according to the
bot's current level.
