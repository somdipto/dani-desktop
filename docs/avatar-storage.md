...
- **Custom** accepts a compatible Images API. Enter its **Base URL**, its
  **Image model** ID, and an optional API key, then choose **Save connection**.
  ...
  it requires. Dani Bot sends only the custom key to this connection.
...
`http://127.0.0.1:20128/v1`. Dani Bot appends `/images/generations`; pasting
that complete endpoint is also accepted. `localhost` and `127.0.0.1` refer to
the machine running the Dani Bot server, including when you open the app
