...
Dani Bot can turn a Linux server you already own into a bot's computer. The agent process stays on your
...
only surface exposed: Dani Bot never opens a public port on the VPS, never stores your SSH key or
...
Dani Bot connects only through a named alias in your `~/.ssh/config` — you type the alias into
...
**Host key first, by hand.** Connect once manually before pointing Dani Bot at the alias:
...
- **No public ports.** The managed container is created with no published ports, and Dani Bot refuses to
...
- **Remove** is yours, done by hand when a bot no longer needs the server:
  `docker -H ssh://my-vps rm -f <container>`. Dani Bot never deletes a container on its own.
