...
Install [Node.js](https://nodejs.org/) 24 or newer, then choose either way to run Dani Bot:
...
- **Managed HTTPS address:** setup asks explicit permission for a public endpoint through Cloudflare and a possible connector download. Device pairing protects chat and settings; the pairing page and basic server identity remain publicly reachable. Sign in to an **Dani Bot account** using an emailed code, or reuse this machine's saved account. This account is separate from ChatGPT, Claude, or an API-provider account. The connection stays active while Dani Bot runs.
...
- **iPhone/iPad:** scan the QR with Camera to open Safari. If you already have the Dani Bot iOS app, use its pairing scanner or paste the full link there.
