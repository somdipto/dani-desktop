...
- **Behind your own proxy or domain:** `npx danibot serve --public-url
  https://bot.example.com`, with the proxy rules from "Putting a proxy in
  front".
...
[Service]
User=danibot
WorkingDirectory=/home/danibot/dani-desktop
Environment=OMB_DATA_DIR=/home/danibot/.danibot
...
that user (`sudo -u danibot claude` etc.) before starting the service.
...
a link like `https://bot.example.com/pair#code=XXXX-XXXX-XXXX`. Open the
