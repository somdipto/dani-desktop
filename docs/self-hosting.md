...
Step by step, for a server you do not have yet: [Deploy Dani Bot on a
...
- a browser for bots, once the engine is installed on the server
  (`npx danibot browser install`, or nothing to do in the Docker image,
...
`npx danibot serve` to start non-interactively with your existing config
...
`npx danibot browser install` as the service user so that user's browser
...
- **A public address, no domain, no proxy, no open port:**
  ...
  ```sh
  npx danibot login          # once: an emailed code signs this machine in
...
- **Your own domain, still one command:**
  ...
  ```sh
  npx danibot serve --domain maus.example.com
  ```
...
```sh
npx danibot service install --domain maus.example.com   # or --tunnel, --tailscale, or nothing
```
...
`npx danibot pair --client` for chat-only access). A code already used by
...
```sh
npx danibot access add her@yourcompany.com
npx danibot access add freelancer@example.com --chat-only
npx danibot access list
```
