...
Run the Dani Bot server anywhere; connect from the desktop app, any
...
2. **Private network helper**: detect a Tailscale MagicDNS name and offer
   ...
   line as `danibot serve --tailscale`.
3. **Managed tunnel**: the cloudflared managed-tunnel channel the companion
   ...
   Shipped for the command line as `danibot login` + `serve --tunnel`
