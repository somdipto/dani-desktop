...
for (const appName of ["openmausbot", "OpenMausBot", "Dani Bot", "DaniBot"]) {
...
try {
  ...
  if (sessionBlocked) {
    ...
    const activeUserData = ["openmausbot", "OpenMausBot", "Dani Bot", "DaniBot"]
    ...
    const userData = ["openmausbot", "OpenMausBot", "Dani Bot", "DaniBot"]
      .map((name) => path.join(xdgConfig, name))
      .find((directory) => existsSync(path.join(directory, "cua-connection.json")));
