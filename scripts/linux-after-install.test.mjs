...
const browserPolicy = path.join(root, "build", "linux-danibot-browser.apparmor");
...
function fixture() {
  ...
  fs.copyFileSync(browserPolicy, path.join(resources, "danibot-browser.apparmor"));
  ...
  fs.writeFileSync(path.join(systemRoot, "apparmor-profiles"), "danibot-browser (unconfined)\n");
...
function runHook(appRoot) {
  return spawnSync("/bin/sh", [hook], {
    encoding: "utf8",
    env: { ...process.env, DANIBOT_POSTINSTALL_TEST_ROOT: appRoot },
  });
}
...
describe("Linux DEB sandbox policy", () => {
  it("allows user namespaces only for the exact installed browser executable", () => {
    ...
    expect(policy).toContain("profile danibot-browser /opt/Dani Bot/resources/browser-engine/chrome/chrome-headless-shell-linux64/chrome-headless-shell flags=(unconfined)");
...
describe.skipIf(process.platform !== "linux")("Linux DEB upgrade hook", () => {
  it("repairs legacy directory and executable modes idempotently", () => {
    ...
    for (let pass = 0; pass < 2; pass += 1) {
      ...
      expect(fs.readFileSync(path.join(apparmorDir, "danibot-browser"), "utf8")).toBe(fs.readFileSync(browserPolicy, "utf8"));
      expect(result.stdout).toContain(`profile operation: -r ${apparmorDir}/danibot-browser`);
    }
  });
  ...
  it("refuses an AppArmor destination symlink without overwriting its target", () => {
    ...
    fs.symlinkSync(external, path.join(apparmorDir, "danibot-browser"));
    const result = runHook(appRoot);
    expect(result.status).not.toBe(0);
  ...
  it("keeps the profile on upgrade and removes only its own policy on uninstall", () => {
    const { appRoot, apparmorDir } = fixture();
    expect(runHook(appRoot).status).toBe(0);
    const profile = path.join(apparmorDir, "danibot-browser");
  ...
  it("removes an already unloaded profile without asking the kernel again", () => {
    ...
    expect(result.stdout).not.toContain("profile operation");
    expect(fs.existsSync(path.join(apparmorDir, "danibot-browser"))).toBe(false);
  });
  ...
  it("keeps the exact policy available for repair if the kernel refuses to unload it", () => {
    ...
    expect(fs.existsSync(path.join(apparmorDir, "danibot-browser"))).toBe(true);
  });
