...
function translationPrompt(source, label, code) {
  return [
    `Translate this JSON UI catalog for Dani Bot, a multi-agent desktop workbench, into ${label} (${code}).`,
    ...
    "Keep placeholders such as {name} exactly, including duplicates. Keep Dani Bot, CLI, and AI unchanged.",
...
function runModel(prompt) {
  const workDir = mkdtempSync(join(tmpdir(), "danibot-locale-"));
  try {
