...
<head>
  ...
  <title>Dani Bot — Cua Driver third-party licenses</title>
...
Dani Bot redistributes two executables from the official Cua Driver ${RELEASE_VERSION} Linux x64 release: \`cua-driver\` and \`cua-cursor-theme\`.
...
The complete MPL-2.0 text and package copyright notices appear in \`THIRD_PARTY_LICENSES.html\`. These files remain under their original licenses; Dani Bot's and Cua's MIT licenses do not replace them.
