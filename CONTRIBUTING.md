...
- stable `Dani Bot-amd64.deb` and `Dani Bot.AppImage` copies used by the latest-download links;
...
Data lives in `~/.danibot/` (bots, transcripts, per-thread NDJSON event logs, config with keys).
...
- **Never touch the real `~/.danibot`.** The setup file points `HOME` at a temp dir; keep it
  that way.
...
API keys are write-only: they land in `~/.danibot/config.json` via `PUT /api/config` and the API
...
Dani Bot release repository, and never change the upstream publish target in a feature PR unless
