...
enum DaniSharedKeychain {
    private static let service = "com.danibot.companion.token"
    ...
    static func remove(_ connectionID: String) -> Bool {
        guard let accessGroup = DaniSharedConfiguration.keychainAccessGroup else {
            return false
        }
    ...
    static func removeIncludingLegacyItem(_ connectionID: String) -> Bool {
        ...
        guard let legacyGroup = DaniSharedConfiguration.legacyAppKeychainAccessGroup else {
            return false
        }
    ...
    private static func requiredAccessGroup() throws -> String {
        guard let accessGroup = DaniSharedConfiguration.keychainAccessGroup else {
            throw DaniSharedKeychainError.configurationMissing
        }
        return accessGroup
    }

    private static func requiredLegacyAccessGroup() throws -> String {
        guard let accessGroup = DaniSharedConfiguration.legacyAppKeychainAccessGroup else {
            throw DaniSharedKeychainError.configurationMissing
        }
        return accessGroup
    }
    ...
    private static func save(
        ...
        guard status == errSecSuccess else {
            throw DaniSharedKeychainError.security(status)
        }
    }
    ...
    private static func readToken(
        ...
        guard let data = item as? Data,
            ...
            throw DaniSharedKeychainError.security(errSecDecode)
        }
        return token
    }
...
enum DaniSharedKeychainError: LocalizedError {
